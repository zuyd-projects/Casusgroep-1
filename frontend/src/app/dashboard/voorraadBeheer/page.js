...
export default function VoorraadBeheerPage() {
  ...
  // Better notification functions
  const showSuccessMessage = (title, message) => {
    // For now using alert, but this could be replaced with a toast library
    alert(`${title}\n\n${message}`);
  };
  ...
  // Update inventory item status via API
  const updateInventoryStatus = async (itemId, status, itemData) => {
    try {
      const updateData = {
        ...itemData,
        status: status,
      ...
      // Note: This would need to be adapted based on your actual inventory API endpoint
      await api.put(`/api/Inventory/${itemId}`, updateData);
    } catch (error) {
      console.error("Error updating inventory status:", error);
    }
  };

  // Toggle status for inventory items
  const handleToggleStatus = async (id) => {
    const item = inventoryData.find((i) => i.id === id);
    const newIsDelivered = !item.isDelivered;
    const newStatus = newIsDelivered ? "Delivered" : "Pending";
      ...
      prev.map((item) =>
        item.id === id
          ? {
              ...
              status: newStatus,
              deliveryDate: newIsDelivered ? currentDate : null,
            }
          : item
      )
    );

    // Update current stock levels if delivered
    if (newIsDelivered && item.expectedBlocks) {
      setCurrentStock((prev) => ({
        Blauw: prev.Blauw + item.expectedBlocks.Blauw,
        Rood: prev.Rood + item.expectedBlocks.Rood,
        Grijs: prev.Grijs + item.expectedBlocks.Grijs,
      }));
    } else if (!newIsDelivered && item.expectedBlocks) {
      // Remove from stock if unmarking as delivered
      setCurrentStock((prev) => ({
        Blauw: Math.max(0, prev.Blauw - item.expectedBlocks.Blauw),
        Rood: Math.max(0, prev.Rood - item.expectedBlocks.Rood),
        Grijs: Math.max(0, prev.Grijs - item.expectedBlocks.Grijs),
      }));
    }

    // Update on server (you may need to implement this based on your API)
    try {
      if (item && item.supplierOrderId) {
        // This would call your API to update the supplier order status
        await updateInventoryStatus(item.supplierOrderId, newStatus, {
          ...item,
          isDelivered: newIsDelivered,
          deliveryDate: newIsDelivered ? currentDate : null,
        });
        ...
        prev.map((item) =>
          item.id === id
            ? {
                ...item,
                isDelivered: !newIsDelivered,
                status: !newIsDelivered ? "Delivered" : "Pending",
                deliveryDate: !newIsDelivered ? currentDate : null,
              }
            : item
        )
  ...
  useEffect(() => {
    if (currentRound) {
      console.log(
        "🔄 Round changed, refetching inventory data for round:",
        currentRound.number
      );
      fetchInventoryData();
    }
  ...
  const fetchInventoryData = async () => {
    try {
      ...
      console.log("📊 Raw supplier orders:", supplierOrders);
      console.log("📊 Raw regular orders:", orders);
      ...
      const inventoryItems = supplierOrders.map((supplierOrder) => {
        ...
        console.log(`📦 Processing supplier order ${supplierOrder.id}:`, {
          supplierOrder,
          relatedOrder,
        ...
        if (relatedOrder && MotorBlockRequirements[relatedOrder.motorType]) {
          ...
          console.log(
            `📊 Block requirements for order ${supplierOrder.id}:`,
            blockRequirements
          ...
          if (
            ...
            console.log(
              `✅ Adding to stock (order ${supplierOrder.id} status: ${supplierOrder.status}):`,
              blockRequirements
            );
            ...
            console.log(
              `⏳ Not adding to stock (order ${supplierOrder.id} status: ${supplierOrder.status})`
            );
          }
        } else {
          console.log(
            `❌ No related order or motor requirements found for supplier order ${supplierOrder.id}`
          );
        }
        return {
      ...
      console.log("📊 Calculated stock levels:", stockLevels);
      console.table(stockLevels);
      console.log("Expected: Blauw: 30, Rood: 36, Grijs: 20");
  ...
  const handleReplacementOrder = async () => {
    ...
    try {
      ...
      // Use the same approach as auto-created supplier orders
      console.log("� Creating replacement order with system approach");
      console.log(" Using hardcoded user ID (same as regular orders):", userId);

      // Create a replacement supplier order
      const orderData = {
        AppUserId: null, // Same as auto-created supplier orders
      ...
      console.log("📦 Sending replacement order:", orderData);
