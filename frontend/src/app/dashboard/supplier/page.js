      ...
      <div className="max-w-[2000px] mx-auto">
        <div className="bg-zinc-100 dark:bg-zinc-900 shadow-md overflow-hidden border-0 rounded-lg">
          <div className="bg-gradient-to-r from-purple-600 via-pink-500 to-purple-500 py-10 px-8 relative">
            ...
            <h1 className="text-4xl font-bold text-white relative z-10">Overzicht Leverancier</h1>
            <p className="text-pink-100 mt-3 text-lg relative z-10">Beheer en monitor alle leveringen van lego blokjes</p>
          </div>
            ...
            <table className="w-full border-separate border-spacing-0">
              <thead>
                <tr className="border-b-2 border-zinc-200 dark:border-zinc-800">
                  <th rowSpan={2} className="px-6 py-5 text-left text-sm font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-wider">
                    ID
                  </th>
                  <th rowSpan={2} className="px-6 py-5 text-left text-sm font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-wider">
                    Tijdstip
                  </th>
                  <th rowSpan={2} className="px-6 py-5 text-center text-sm font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-wider">
                    Periode
                  </th>
                  <th colSpan={3} className="px-6 py-5 text-center text-sm font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-wider bg-zinc-50 dark:bg-zinc-900/50">
                    Bestelling
                  </th>
                  <th rowSpan={2} className="px-6 py-5 text-center text-sm font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-wider">
                    Geleverd?
              ...
              <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
                {orderRounds.length === 0 ? (
                  ...
                  [...orderRounds].reverse().map((r, idx) => (
                    ...
                    >
                      <td className="px-6 py-5 whitespace-nowrap text-base font-bold text-white">{r.id}</td>
                      <td className="px-6 py-5 whitespace-nowrap text-base text-zinc-600 dark:text-zinc-400">{r.timestamp || "-"}</td>
                      <td className="px-6 py-5 text-center">
                        <span className="inline-flex items-center justify-center h-10 w-10 rounded-full font-bold text-lg text-white">{r.round}</span>
                      </td>
                      {legoColors.map((color) => (
                      ...
                      <td className="px-6 py-5 text-center">
                        <button
                          ...
                          title={r.geleverdVinkje ? "Levering ontvangen" : "Nog niet geleverd"}
                        >
                          {r.geleverdVinkje ? (
                      ...
                      <td className="px-6 py-5 text-center text-base">
                        <span className="inline-flex items-center px-4 py-2 rounded-full text-base font-medium bg-gradient-to-r from-purple-500/70 to-pink-500/70 dark:from-purple-600/60 dark:to-pink-600/60 text-white shadow-sm backdrop-blur-sm">
                          Periode {r.geleverdInPeriode ? r.geleverdInPeriode : "-"}
                        </span>
                      </td>
                      <td className="px-6 py-5 text-base">
                        <button
                          ...
                          onClick={() => {
                            const newNote = window.prompt("Voeg notitie toe", r.note || "");
                            if (newNote !== null) {
                        ...
                        {r.note && (
                          <div className="mt-2 text-sm text-zinc-500 dark:text-zinc-400 max-w-xs truncate" title={r.note}>
                            {r.note}
                          </div>
                        )}
                      </td>
