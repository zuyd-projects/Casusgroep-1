                    ...
                    {journeyData.caseJourneys.map((journey, journeyIndex) => (
                        <Card key={journeyIndex} title={`Case Journey: ${journey.caseId}`}>
                            <div className="mb-4 grid grid-cols-1 md:grid-cols-4 gap-4">
                                ...
                                <div className={`text-center p-3 rounded-lg ${journey.isSuccessful
                                        ? 'bg-green-50 dark:bg-green-900/20'
                                        : 'bg-red-50 dark:bg-red-900/20'
                                    }`}>
                                    <div className={`text-lg font-bold ${journey.isSuccessful ? 'text-green-600' : 'text-red-600'
                                        }`}>
                                        {journey.isSuccessful ? 'Success' : 'Failed'}
                                    </div>
                                    <div className={`text-sm ${journey.isSuccessful ? 'text-green-500' : 'text-red-500'
                                        }`}>
                                        Status
                                    </div>
                                </div>
                                <div className={`text-center p-3 rounded-lg ${journey.hasRework
                                        ? 'bg-yellow-50 dark:bg-yellow-900/20'
                                        : 'bg-green-50 dark:bg-green-900/20'
                                    }`}>
                                    <div className={`text-lg font-bold ${journey.hasRework ? 'text-yellow-600' : 'text-green-600'
                                        }`}>
                                        {journey.hasRework ? 'Yes' : 'No'}
                                    </div>
                                    <div className={`text-sm ${journey.hasRework ? 'text-yellow-500' : 'text-green-500'
                                        }`}>
                                        Rework
                                    </div>
                                </div>
