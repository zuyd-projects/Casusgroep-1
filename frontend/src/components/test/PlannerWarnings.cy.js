...
describe('<PlannerWarnings />', () => {
  beforeEach(() => {
    ...
    cy.intercept('GET', '/api/ProcessMining/delivery-predictions', mockNoWarnings).as('getDeliveryPredictions');
  });
  ...
  it('shows "All deliveries on track" when there are no warnings (default)', () => {
    cy.intercept('GET', '/api/ProcessMining/delivery-predictions', mockNoWarnings).as('getNoPredictions');
  ...
  it('shows "All deliveries on track" in compact mode when there are no warnings', () => {
    cy.intercept('GET', '/api/ProcessMining/delivery-predictions', mockNoWarnings).as('getNoWarningsCompact');
  ...
  it('renders warnings and stats when there are warnings', () => {
    cy.intercept('GET', '/api/ProcessMining/delivery-predictions', mockWarnings).as('getWarnings');
    ...
    cy.contains('3+ Rounds Delayed').should('be.visible');
    ...
    cy.contains('3.2').should('be.visible'); // averageDeliveryTime
  ...
  it('renders compact warnings view', () => {
    cy.intercept('GET', '/api/ProcessMining/delivery-predictions', mockWarnings).as('getWarningsCompact');
  ...
  it('toggles warnings visibility in full mode', () => {
    cy.intercept('GET', '/api/ProcessMining/delivery-predictions', mockWarnings).as('getWarningsToggle');
    ...
    cy.contains('Active Delivery Warnings').should('be.visible');    cy.contains('Late Delivery').should('not.exist');
  ...
  it('shows correct warning counts and severity badges', () => {
    cy.intercept('GET', '/api/ProcessMining/delivery-predictions', mockWarnings).as('getWarningsCount');
  ...
  it('displays correct data in compact mode with high priority warnings only', () => {
    ...
    cy.intercept('GET', '/api/ProcessMining/delivery-predictions', mockMixedWarnings).as('getMixedWarnings');
  ...
  it('handles empty response gracefully', () => {
    cy.intercept('GET', '/api/ProcessMining/delivery-predictions', {}).as('getEmpty');
  ...
  it('updates when simulation context changes', () => {
    // Mock first response
    cy.intercept('GET', '/api/ProcessMining/delivery-predictions', mockNoWarnings).as('getInitial');
  ...
  it('displays correct time-based information', () => {
    cy.intercept('GET', '/api/ProcessMining/delivery-predictions', mockWarnings).as('getTimeInfo');
    ...
    cy.contains('3 rounds').should('be.visible'); // orderRoundAge
    cy.contains('2 rounds').should('be.visible'); // orderRoundAge for second warning
    cy.contains('2 rounds past deadline').should('be.visible'); // roundsDelay
    cy.contains('Expected by Round 5').should('be.visible'); // expectedDeliveryRound
  });

  it('shows N/A for missing average delivery time', () => {
    const mockNoAvgTime = {
      ...mockWarnings,
      averageDeliveryTime: null
    };

    cy.intercept('GET', '/api/ProcessMining/delivery-predictions', mockNoAvgTime).as('getNoAvg');
