  ...
  describe("Status-Specific Styling", () => {
    const statusTests = [
      ...
      },
      {
        ...
        expectedText: "Toproduction",
        description: "to production status with blue styling",
      },
      {
        status: "inproduction",
        ...
        description: "in production status with green styling",
      },
      {
        status: "delivered",
      ...
      },
      {
        ...
        description:
          "awaiting account manager approval status with orange styling",
      },
      {
    ...
    statusTests.forEach(
      ({ status, bgClass, textClass, expectedText, description }) => {
  ...
  describe("Text Formatting", () => {
    it("handles camelCase status formatting correctly", () => {
      const camelCaseTests = [
        {
          input: "awaitingAccountManagerApproval",
          expected: "Awaiting Account Manager Approval",
        },
        {
          input: "rejectedByAccountManager",
          expected: "Rejected By Account Manager",
        },
        {
          input: "approvedByAccountManager",
          expected: "Approved By Account Manager",
        },
        { input: "inProduction", expected: "In Production" },
        { input: "toProduction", expected: "To Production" },
        { input: "productionError", expected: "Production Error" },
      ];
    ...
    it("handles compound status names correctly", () => {
      const compoundTests = [
        {
          input: "rejectedbyvoorraadbeheer",
          expected: "Rejectedbyvoorraadbeheer",
        },
        {
          input: "awaitingaccountmanagerapproval",
          expected: "Awaitingaccountmanagerapproval",
        },
        {
          input: "approvedbyaccountmanager",
          expected: "Approvedbyaccountmanager",
        },
        {
          input: "rejectedbyaccountmanager",
          expected: "Rejectedbyaccountmanager",
        },
        { input: "productionerror", expected: "Productionerror" },
      ];

      compoundTests.forEach(({ input, expected }) => {
        cy.mount(<StatusBadge status={input} />);
        cy.get("span").should("contain.text", expected);
      });
    });
    ...
    it("handles case insensitive status matching", () => {
      const caseTests = [
        { input: "PENDING", styling: "bg-yellow-100" },
        { input: "Pending", styling: "bg-yellow-100" },
        { input: "pending", styling: "bg-yellow-100" },
        { input: "COMPLETED", styling: "bg-violet-100" },
        { input: "Completed", styling: "bg-violet-100" },
        { input: "ToProduction", styling: "bg-blue-100" },
        { input: "TOPRODUCTION", styling: "bg-blue-100" },
      ];

      caseTests.forEach(({ input, styling }) => {
        cy.mount(<StatusBadge status={input} />);
        cy.get("span").should("have.class", styling);
      });
    });
